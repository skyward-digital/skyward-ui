...
export const Badge = ({
  ...
  return (
    <div
      className={cn(
        // layout
        'border inline-flex items-center justify-center overflow-hidden whitespace-nowrap font-semibold tracking-wide [&>*]:inline-flex [&>*]:items-center',
        ...
        // Icon sizes
        size === 'sm' && '[&>svg]:-mx-0.5 [&>svg]:h-3 [&>svg]:w-3 [&>*>svg]:-mx-0.5 [&>*>svg]:h-3 [&>*>svg]:w-3',
        size === 'md' && '[&>svg]:-mx-0.5 [&>svg]:h-4 [&>svg]:w-4 [&>*>svg]:-mx-0.5 [&>*>svg]:h-4 [&>*>svg]:w-4',
        size === 'lg' && '[&>svg]:-mx-0.5 [&>svg]:h-5 [&>svg]:w-5 [&>*>svg]:-mx-0.5 [&>*>svg]:h-5 [&>*>svg]:w-5',
    ...
    >
      ...
      {image?.src && (
        <Image src={image.src} alt={image.alt} className={cn('flex-shrink-0 rounded-full -ml-0.5 w-4 h-4')} />
      )}
      {children}
    </div>
  )
